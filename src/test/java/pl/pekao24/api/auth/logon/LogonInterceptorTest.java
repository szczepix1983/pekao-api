...
class LogonInterceptorTest {
    ...
    void intercept() throws Exception {
        ...
        LogonInterceptor interceptor = new LogonInterceptor(httpHeadersCache);
        interceptor.intercept(request, new byte[]{}, clientHttpRequestExecution);

        verify(requestHeaders, times(1)).set(eq(HttpHeaders.COOKIE), eq("cookies"));
